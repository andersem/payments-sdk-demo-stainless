...
export class SessionURLCallback extends APIResource {
  /**
   * This API endpoint on the merchant side allows Dintero to notify the
   * `session.url.callback_url` when the payment is completed
   *
   ...
   */
  create(params: SessionURLCallbackCreateParams, options?: Core.RequestOptions): Core.APIPromise<unknown> {
    const {
      includes,
      query_merchant_reference,
      method,
      time,
      transaction_id,
      delay_callback,
      ...
      query_session_id,
      sid,
      body_merchant_reference,
      body_session_id,
      'Dintero-Signature': dinteroSignature,
      ...body
    } = params;
    return this._client.post('/examples/session_url_callback', {
      query: {
        includes,
        merchant_reference: query_merchant_reference,
        method,
        time,
        transaction_id,
        delay_callback,
      ...
      body: { merchant_reference: body_merchant_reference, session_id: body_session_id, ...body },
      ...options,
      headers: {
        ...(dinteroSignature != null ? { 'Dintero-Signature': dinteroSignature } : undefined),
        ...options?.headers,
      },
    });
  }
   ...
   */
  retrieve(
    params: SessionURLCallbackRetrieveParams,
    options?: Core.RequestOptions,
  ): Core.APIPromise<unknown> {
    const { 'Dintero-Signature': dinteroSignature, ...query } = params;
    return this._client.get('/examples/session_url_callback', {
      query,
      ...options,
      headers: {
        ...(dinteroSignature != null ? { 'Dintero-Signature': dinteroSignature } : undefined),
...
export type SessionURLCallbackCreateResponse = unknown;

export type SessionURLCallbackRetrieveResponse = unknown;

export interface SessionURLCallbackCreateParams {
  /**
   * Query param: Aditional data included
   */
  includes: string;

  /**
   * Query param: The merchants reference
   */
  query_merchant_reference: string;

  /**
   * Query param: POST method used to deliver the callback
   */
  method: string;

  /**
   * Query param: ISO 8601 format for when the transaction was created
   */
  time: string;

  /**
   * Query param: The Id for the transaction created
   */
  transaction_id: string;

  /**
   * Body param: Non-negative, minor units. Total amount of the transaction
   */
  amount: number;

  /**
   * Body param: ISO 4217 transaction currency
   */
  currency: string;

  /**
   * Body param: The payment product corresponding to this transaction
   */
  payment_product:
    | 'bambora'
    | 'collector'
  ...
  /**
   * Body param: The payment product type corresponding to this transaction
   */
  ...
  /**
   * Query param: Delay before delivering the callback
   */
  ...
  /**
   * Query param: Event applied to transaction
   */
  event?: string;

  /**
   * Query param: Id for the event applied to transaction
   */
  event_id?: string;

  /**
   * Query param: Report error callback
   */
  report_error?: boolean;

  /**
   * Query param: Deliver callback on othe transaction events
   */
  report_event?: string;

  /**
   * Query param: Session Id. Either `session_id` or `sid` is required.
   */
  query_session_id?: string;

  /**
   * Query param: Session Id if `sid_parameter_name=sid`. Either `session_id` or
   * `sid` is required.
   */
  sid?: string;

  /**
   * Body param: An ID that uniquely identifies the resource
   */
  id?: string;
   ...
   * Body param: Address
   */
  billing_address?: SessionURLCallbackCreateParams.BillingAddress;

  /**
   * Body param:
   */
  card?: SessionURLCallbackCreateParams.Card;

  /**
   * Body param: Configuration for checkboxes that should be part of the checkout
   */
  checkboxes?: Array<SessionURLCallbackCreateParams.Checkbox>;

  /**
   * Body param: When the transaction was created
   */
  created_at?: string;
   ...
   * Body param:
   */
  customer?: SessionURLCallbackCreateParams.Customer;

  /**
   * Body param: The IP address of the customer
   */
  customer_ip?: string;

  /**
   * Body param: A short reference / descriptor that will show up on the customers
   * bank statement
   */
   ...
   * Body param:
   */
  events?: Array<SessionURLCallbackCreateParams.Event>;

  /**
   * Body param: The gift cards that used to partially or fully authorize the
   * transaction
   */
  gift_cards?: Array<SessionURLCallbackCreateParams.GiftCard>;

  /**
   * Body param:
   */
  initiating_system_request_headers?: SessionURLCallbackCreateParams.InitiatingSystemRequestHeaders;

  /**
   * Body param: The applicable transaction items
   */
  items?: Array<SessionURLCallbackCreateParams.Item>;

  /**
   * Body param: A reference specified by the merchant to identify the transaction
   */
  body_merchant_reference?: string;
   ...
   * Body param:
   */
  metadata?: SessionURLCallbackCreateParams.Metadata;

  /**
   * Body param: Initiated by the merchant or used to generate a token
   */
  payment_operation?: 'unscheduled_purchase' | 'recurring_purchase' | 'generate_payment_token';

  /**
   * Body param: An id that identifies the seller, value will be included in the
   * settlement reports
   */
  ...
  /**
   * Body param: The session id for the transaction
   */
  body_session_id?: string;

  /**
   * Body param: Overall settlement status after the events
   */
   ...
   * Body param: Address
   */
  shipping_address?: SessionURLCallbackCreateParams.ShippingAddress;

  /**
   * Body param:
   */
  shipping_option?: SessionURLCallbackCreateParams.ShippingOption;

  /**
   * Body param:
   */
  store?: SessionURLCallbackCreateParams.Store;

  /**
   * Body param: When the transaction was last modified.
   */
  updated_at?: string;
   ...
   * Body param:
   */
  url?: SessionURLCallbackCreateParams.URL;

  /**
   * Body param: The full user agent string of the device the customer used to submit
   * the transaction
   */
  user_agent?: string;

  /**
   * Header param: Dintero signature that can be used to verify the payload from the
   * callback.
   *
...
export namespace SessionURLCallbackCreateParams {
  /**
   * Address
   */
...
export interface SessionURLCallbackRetrieveParams {
  /**
   * Query param: The merchants reference
   */
  ...
  /**
   * Query param: The method to use when delivering the callback
   */
  method?: string;
...
export declare namespace SessionURLCallback {
  export {
