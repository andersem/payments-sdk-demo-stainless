...
export class Sessions extends APIResource {
  ...
  retrieve(sessionId: string, options?: Core.RequestOptions): Core.APIPromise<SessionRetrieveResponse>;
  retrieve(
    sessionId: string,
    ...
    if (isRequestOptions(query)) {
      return this.retrieve(sessionId, {}, query);
    }
    return this._client.get(`/sessions/${sessionId}`, { query, ...options });
  }
   ...
   * **Requirements**:
   *
   * - `order.shipping_option` must be included in `express_shipping_options` if both
   *   are set.
  ...
  update(
    ...
    const {
      ...
      update_without_lock,
      'Dintero-Feature-Toggles': dinteroFeatureToggles,
    ...
    return this._client.put(`/sessions/${sessionId}`, {
      query: { force_shipping_address_callback, update_without_lock },
      body,
      ...options,
      headers: {
        ...(dinteroFeatureToggles?.toString() != null ?
          { 'Dintero-Feature-Toggles': dinteroFeatureToggles?.toString() }
        : undefined),
   ...
   * Cancel a session
   *
   * The session transaction will be voided in case where it is initialized or
   * authorized.
   *
   * Cancel is not allowed in case where the current transaction state is not
  ...
  cancel(
    ...
    options?: Core.RequestOptions,
  ): Core.APIPromise<SessionCancelResponse>;
  cancel(sessionId: string, options?: Core.RequestOptions): Core.APIPromise<SessionCancelResponse>;
  cancel(
    sessionId: string,
    params: SessionCancelParams | Core.RequestOptions = {},
    options?: Core.RequestOptions,
  ): Core.APIPromise<SessionCancelResponse> {
    if (isRequestOptions(params)) {
      return this.cancel(sessionId, {}, params);
    }
    const {
      'Dintero-System-Name': dinteroSystemName,
      'Dintero-System-Plugin-Name': dinteroSystemPluginName,
    ...
    return this._client.post(`/sessions/${sessionId}/cancel`, {
      ...options,
      headers: {
        ...(dinteroSystemName != null ? { 'Dintero-System-Name': dinteroSystemName } : undefined),
        ...(dinteroSystemPluginName != null ?
   ...
   * >
   * > 200 response will be returned when request fails due to authorization error.
   * > Transaction will have status `FAILED` and error information can be found at
   * > `events.error`.
   * >
   ...
   * - write:checkout
   */
  pay(params: SessionPayParams, options?: Core.RequestOptions): Core.APIPromise<SessionPayResponse> {
    const {
      'Dintero-Feature-Toggles': dinteroFeatureToggles,
      'Dintero-System-Name': dinteroSystemName,
      'Dintero-System-Plugin-Name': dinteroSystemPluginName,
    ...
    return this._client.post('/sessions/pay', {
      body,
      ...options,
      headers: {
        ...(dinteroFeatureToggles?.toString() != null ?
  ...
  /**
   * This endpoint lets you create payment and recurrence tokens without reserving or
   * charging any amount.
   *
   * The URL returned by this endpoint opens a web site where the customer can enter
   * their payment details, e.g. card information.
   *
   * The payment details will be validated and a transaction with a
   * payment/recurrence token will be created on success containing the payment token
   * created from the customer payment details.
   *
   ...
   */
  paymentToken(
    params: SessionPaymentTokenParams,
    options?: Core.RequestOptions,
  ): Core.APIPromise<SessionPaymentTokenResponse> {
    const {
      include_session,
      'Dintero-System-Name': dinteroSystemName,
      'Dintero-System-Plugin-Name': dinteroSystemPluginName,
    ...
    return this._client.post('/sessions/payment-token', {
      query: { include_session },
      body,
      ...options,
      headers: {
        ...(dinteroSystemName != null ? { 'Dintero-System-Name': dinteroSystemName } : undefined),
...
export interface Session {
   ...
   */
  checkboxes?: Array<Session.Checkbox>;
  ...
  initiating_system_request_headers?: Session.InitiatingSystemRequestHeaders;
...
export namespace Session {
  ...
  export namespace Configuration {
    ...
    export namespace Instabank {
      ...
      export interface Installment {
        ...
        products?: Array<Installment.Product>;
      ...
      export interface Postponement {
        ...
        products?: Array<Postponement.Product>;
    ...
    export interface Payout {
       ...
       */
      payment_products?: Array<Payout.PaymentProduct>;
    }
  ...
  export interface Order {
    ...
    /**
     * The origin amount to authorize/capture including VAT before any discount, only
     * set if the session was updated when calculating discounts.
     *
...
export interface SessionPayResult {
  ...
  actions?: Array<SessionPayResult.Action>;

  session_id?: string;
}

export namespace SessionPayResult {
...
export interface SessionRead {
  configuration?: SessionRead.Configuration;

  customer?: SessionRead.Customer;

  /**
   * metadata about the session
   */
  metadata?: SessionRead.Metadata;

  order?: SessionRead.Order;
}
  ...
  export namespace Customer {
    ...
    export namespace Tokens {
      export interface PayexCreditcard {
        /**
         * Id included if the payex.creditcard was created with payment_token set.
         */
...
export interface SessionRetrieveResponse extends ID, Session, SessionRead, SessionCancelled {}

export interface SessionUpdateResponse extends ID, Session, SessionRead, SessionCancelled {}

export interface SessionCancelResponse extends ID, Session, SessionRead, SessionCancelled {}

export interface SessionPayResponse extends TransactionsAPI.Transaction, SessionPayResult {}

export interface SessionPaymentTokenResponse {
  /**
  ...
  session?: SessionPaymentTokenResponse.Session;
...
export namespace SessionPaymentTokenResponse {
  export interface Session
    extends SessionsAPI.ID,
      SessionsAPI.Session,
      SessionsAPI.SessionRead,
      SessionsAPI.SessionCancelled {}
}
...
export interface SessionUpdateParams {
   ...
   * Body param:
   */
  order: SessionUpdateParams.Order;

  /**
   * Query param: If `express.shipping_options` is set, there will not be a callback
   * to `shipping_address_callback_url`, unless `force_shipping_address_callback` is
   * also set.
   *
   * If `express.shipping_options` is not set, there will be a callback to
   * `shipping_address_callback_url`.
   */
  force_shipping_address_callback?: boolean;

  /**
   * Query param: Allow updating session without it having been locked first.
   *
   ...
   * Body param:
   */
  express?: SessionUpdateParams.Express;

  /**
   * Body param: Remove lock after updating
   */
  remove_lock?: boolean;

  /**
   * Header param: Feature toggles that will change how the API works.
   *
   * These feature toggles are usually the preferred way to use the API, but they
   * break the current API.
   *
   * - strict-session-amounts: `order.amount` must equal the sum of amounts in
   *   `order.items` + `order.shipping_option.amount` when creating sessions
   */
  'Dintero-Feature-Toggles'?: Array<'strict-session-amounts'>;
...
export interface SessionPayParams {
  /**
   * Body param:
   */
  payment?: SessionPayParams.Payment;

  /**
   * Body param: The session to create the payment from
   */
  session?: SessionPayParams.Session;

  /**
   * Header param: - **`strict-merchant-reference`**: The
   * `session.order.merchant_reference` must be unique. The pay request will fail
   * with `400 BadRequest` error if `merchant_reference` is duplicated by existing
   * session.
   *
   * - **`strict-success-merchant-reference`**: The
   *   `session.order.merchant_reference` must be unique. The pay request will fail
   *   with `400 BadRequest` error if `merchant_reference` is duplicated by existing
   *   session that was successfully authorized. This flag is less strict than
   *   **`strict-merchant-reference`**, allowing for duplicates session if previous
   *   session failed
   */
  'Dintero-Feature-Toggles'?: Array<'strict-merchant-reference' | 'strict-success-merchant-reference'>;
...
export namespace SessionPayParams {
  ...
  export namespace Session {
    ...
    export namespace Customer {
      export interface Tokens {
        'bambora.creditcard'?: Tokens.BamboraCreditcard;

        'payex.creditcard'?: Tokens.PayexCreditcard;
      }

      export namespace Tokens {
        export interface BamboraCreditcard {
          /**
           * Preload the store payment data related to the payment token and let the customer
           * make a purchase without having to enter all card data
           *
           * - The `bambora.creditcard` must be enabled in the session configuration to
           *   activate the use of provided payment token
           * - Use the `bambora.creditcard.generate_payment_token` option to generate the
           *   token.
           */
          payment_token?: string;

          /**
           * Preload the store payment data related to the recurrence token and let the
           * customer make a purchase without having to enter all card data
           *
           * - The `bambora.creditcard` must be enabled in the session configuration to
           *   activate the use of provided payment token
           * - Use the `bambora.creditcard.generate_recurrence_token` option to generate the
           *   token.
           */
        ...
        export interface PayexCreditcard {
           ...
           * make a purchase without having to enter all card data
           *
           * - The `payex.creditcard` must be enabled in the session configuration to
           *   activate the use of provided payment token
           * - Use the `payex.creditcard.generate_payment_token` option to generate the
           *   token.
           */
          payment_token?: string;
           ...
           * - The `payex.creditcard` must be enabled in the session configuration to
           *   activate the use of provided payment token
           * - Use the `payex.creditcard.generate_recurrence_token` option to generate the
           *   token.
           */
          recurrence_token?: string;
        }
...
export interface SessionPaymentTokenParams {
  /**
   * Body param: The session to create the payment token from
   */
  session: SessionPaymentTokenParams.Session;

  /**
   * Body param:
   */
  token_provider: SessionPaymentTokenParams.TokenProvider;

  /**
   * Query param: Include all details about the session created
   */
...
export namespace SessionPaymentTokenParams {
  /**
