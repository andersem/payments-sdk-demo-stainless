...
const pkgImportPath = process.env['PKG_IMPORT_PATH'] ?? 'spec-payments-stainless-demo/';
...
async function postprocess() {
  for await (const file of walk(path.resolve(__dirname, '..', '..', 'dist'))) {
    ...
    if (file.endsWith('.d.ts')) {
      ...
      // if we have `import { type Readable } from 'spec-payments-stainless-demo/_shims/index'`,
